...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        # self.pos_embeddings_DEP = nn.Embedding(
        #   self.pos_size, hps['pos_edim'])
        ...
        self.argument_map = nn.Sequential(
            nn.Linear(2 * lstm_hidden_dim, self.argument_size),
            nn.LeakyReLU(0.1))
        self.predicate_map = nn.Sequential(
            nn.Linear(2 * lstm_hidden_dim, self.argument_size),
            nn.LeakyReLU(0.1))
        self.W_R = nn.Linear(in_features=role_embedding_dim * 2,
                             out_features=(self.argument_size + 1)**2)

        # Dropouts
        self.SRL_input_dropout = nn.Dropout(p=0.3)
        self.DEP_input_dropout = nn.Dropout(p=0.3)
        self.hidden_state_dropout = nn.Dropout(p=0.3)
        self.mlp_pred_dropout = nn.Dropout(p=0.2)
        self.mlp_argu_dropout = nn.Dropout(p=0.2)
        ...
        self.num_layers = 3
        srl_input_size = sent_embedding_dim_SRL + \
            self.elmo_emb_size * 1 + 2 * self.pos_size
    ...
    def init_hidden_share(self):
        ...
        return (torch.zeros(self.num_layers * 2, self.batch_size,
                            self.hidden_dim, requires_grad=False).to(device),
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in,
        ...
        fixed_embeds_DEP = fixed_embeds_DEP.view(
            self.batch_size, len(sentence[0]), self.word_emb_dim)

        embeds_forDEP = torch.cat(
            (embeds_DEP, fixed_embeds_DEP, pos_embeds, region_marks), 2)
        ...
        dep_tag_space_use = self.MLP(F.tanh(
            self.hidden2tag(Label_features))).view(
                len(sentence[0]) * self.batch_size, -1)
        ...
        dep_tag_space_spe = self.MLP_spe(self.link_dropout(F.tanh(
            self.hidden2tag_spe(Label_features)))).view(
                len(sentence[0]) * self.batch_size, -1)
        dep_tag_space_spe_use = self.MLP_spe(F.tanh(
            self.hidden2tag_spe(Label_features))).view(
                len(sentence[0]) * self.batch_size, -1)
        TagProbs_use = F.softmax(dep_tag_space_spe_use, dim=1).view(
            self.batch_size, len(sentence[0]), -1)
        # construct SRL input
        TagProbs_noGrad = TagProbs_use.detach()
        ...
        hidden_states_argument = self.mlp_argu_dropout(
            self.argument_map(hidden_states))
        hidden_states_argument = hidden_states_argument.view(
            self.batch_size * len(sentence[0]), -1)
        hidden_states_argument = torch.cat(
            (hidden_states_argument, torch.ones(self.batch_size *
                                                len(sentence[0]), 1,
                                                requires_grad=True
                                                ).to(device)),
            1)
        ...
        predicate_embeds = self.mlp_pred_dropout(
            self.predicate_map(predicate_embeds))
        predicate_embeds = predicate_embeds.view(self.batch_size, -1)
        # B H+1
        predicate_embeds = torch.cat((predicate_embeds, torch.ones(
            self.batch_size,  1, requires_grad=True).to(device)), 1)
        ...
        # B nr H+1 H+1
        mapped_roles = F.relu(self.W_R(role_embeds)).view(
            self.batch_size, -1, self.argument_size+1, self.argument_size+1)
        # B H+1 nr*H+1
        mapped_roles = torch.transpose(mapped_roles, 1, 2)
        mapped_roles = mapped_roles.contiguous().view(
            self.batch_size, self.argument_size+1, -1)

        # B T nr*H+1
        lin = torch.matmul(hidden_states_argument, mapped_roles)
        # B T*nr H+1
        lin = lin.view(self.batch_size, -1, self.argument_size+1)
        predicate_embeds = predicate_embeds.view(
            self.batch_size, self.argument_size+1, 1)
