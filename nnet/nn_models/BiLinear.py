...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.argument_map = nn.Sequential(nn.Linear(2 * lstm_hidden_dim, self.argument_size), nn.LeakyReLU(0.1))
        self.predicate_map = nn.Sequential(nn.Linear(2 * lstm_hidden_dim, self.argument_size), nn.LeakyReLU(0.1))
        self.W_R = nn.Linear(in_features=role_embedding_dim * 2, out_features=(self.argument_size + 1)*(self.argument_size + 1))
    ...
    def init_hidden_share(self):
        ...
        return (torch.zeros(self.num_layers * 2, self.batch_size, self.hidden_dim, requires_grad=False).to(device),
                torch.zeros(self.num_layers * 2, self.batch_size, self.hidden_dim, requires_grad=False).to(device))
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        hidden_states_argument = self.mlp_argu_dropout(self.argument_map(hidden_states))
        hidden_states_argument = hidden_states_argument.view(self.batch_size * len(sentence[0]), -1)
        hidden_states_argument = torch.cat(
            (hidden_states_argument, torch.ones(self.batch_size * len(sentence[0]), 1, requires_grad=True).to(device)),
            1)
        hidden_states_argument = hidden_states_argument.view(self.batch_size, len(sentence[0]), -1)
