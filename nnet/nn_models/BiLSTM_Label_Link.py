...
class BiLSTMTagger(nn.Module):
    ...
    def init_hidden_share(self):
        ...
        return (torch.zeros(2 * 2, self.batch_size, self.hidden_dim,
                            requires_grad=False),
                torch.zeros(2 * 2, self.batch_size, self.hidden_dim,
                            requires_grad=False))
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in,
        ...
        embeds = embeds.view(self.batch_size, len(
            sentence[0]), self.word_emb_dim)
        ...
        # embeds = torch.cat((embeds, fixed_embeds, pos_embeds, region_marks),
        #   2)
        ...
        hidden_states, self.hidden = self.BiLSTM_share(
            embeds_sort, self.hidden)
        ...
        concat_embeds = torch.zeros(
            bf_e.size()[0], bf_e.size()[1], bf_e.size()[2])
        ...
        # hidden states [time_steps * batch_size * hidden_units]
        hidden_states, self.hidden_2 = self.BiLSTM_Spe(
        ...
        Label_feature = F.relu(
            self.Label_hidden2hidden(F.tanh(label_feature_spe)))

        hidden_states = torch.cat((hidden_states, Label_feature), 2)
        ...
        embeds_sort = rnn.pack_padded_sequence(
            embeds_sort, lengths_sort.cpu().numpy(), batch_first=True)
        # hidden states [time_steps * batch_size * hidden_units]
        hidden_states, self.hidden_3 = self.BiLSTM_SRL(
        ...
        # T * B * H
        added_embeds = Variable(torch.zeros(hidden_states_3.size(
        )[1], hidden_states_3.size()[0], hidden_states_3.size()[2]))
